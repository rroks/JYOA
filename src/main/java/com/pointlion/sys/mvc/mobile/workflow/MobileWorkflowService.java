 ...
 *
 */
public class MobileWorkflowService extends NoticeService {
	public static final MobileWorkflowService me = new MobileWorkflowService();
	 ...
	 * 获取个人单据
	 */
	public Page<Record> getUserDocumentList2(String pageNum, String pageSize, String flowType, String defKey, String docType, String userId, String type, String searchText) {
		String tableName = WorkFlowUtil.getTablenameByDefkey(defKey);
		Page<Record> pageList = null;
		if("CC".equals(flowType)){
			pageList = getFlowCCPage2(pageNum,pageSize,defKey,userId,searchText);
		}else{
			if ("1".equals(type)) {
				tableName = OaApplyCustom.tableName;
			}
			...
			String sql = "from " + tableName + " o LEFT JOIN act_hi_procinst p ON o.proc_ins_id=p.ID_ where o."
					+ userColumn + "='" + userId + "'";
			String sqlEXT = "";
			if (StrKit.notBlank(docType)) {
				sql = sql + " and o.type='" + docType + "'";
			...
			if("YWC".equals(flowType)){
				sql = sql + "and o.if_complete ='1'";
			}
			if("SPZ".equals(flowType)){
				sql = sql + "and o.if_complete='0' and o.if_submit='1'";
			}
			if("WTJ".equals(flowType)){
				sql = sql + "and o.if_complete='0' and o.if_submit='0'";
			}
			String tableType = defKey.split("_")[0];
			if(StrKit.notBlank(searchText)){
				if("ContractApply".equals(tableType)){
					sql = sql + " and (o.applyer_name like '%"+searchText+"%' or o.org_name like '%"+searchText+"%'"
							+ " or o.des like '%"+searchText+"%' or o.title like '%"+searchText+"%' or o.name like '%"+searchText+"%'"
									+ " or o.custom_name like '%"+searchText+"%' or o.custom_contact_person like '%"+searchText+"%')";
				}else if("Finance".equals(tableType)){
					sql = sql + " and (o.applyer_name like '%"+searchText+"%' or o.org_name like '%"+searchText+"%'"
							+ " or o.title like '%"+searchText+"%' or o.des like '%"+searchText+"%' or o.receive_money_org_name like '%"+searchText+"%'"
									+ " or o.tax_price_name like '%"+searchText+"%' or o.common_price_name like '%"+searchText+"%'"
											+ "or  o.change_des like '%"+searchText+"%' or o.formproject_name like '%"+searchText+"%' "
													+ "or o.about_orgname like '%"+searchText+"%' )";
				}else{
					//des\reason\open_bank\account_type\legel_seal\finance_seal\account_name\account_num\formproject_name
						sql = sql + " and (o.applyer_name like '%"+searchText+"%' or o.org_name like '%"+searchText+"%' "
								+ "or o.reason like '%"+searchText+"%' or o.des like '%"+searchText+"%' or o.open_bank like '%"+searchText+"%'"
										+ "or o.account_type like '%"+searchText+"%' or o.account_name like '%"+searchText+"%' or o.account_num like '%"+searchText+"%'"
												+ " or o.formproject_name like '%"+searchText+"%')";
				}
			}
			
			sql = sql + " order by o.create_time desc";
			pageList = Db.paginate(Integer.valueOf(pageNum), Integer.valueOf(pageSize), "select * ", sql);
		}
	 ...
	 * --管理页面使用
	 */
	public Page<Record> getFlowCCPage2(String pageNum, String pageSize, String defkey ,String userid,String searchText){
		String tableName = WorkFlowUtil.getTablenameByDefkey(defkey);
		if(StrKit.isBlank(tableName)){
			tableName = OaApplyCustom.tableName;
		}
		String sql = "from "+tableName+" o, oa_flow_carbon_c  cc, act_hi_procinst p  where o.proc_ins_id=p.ID_ and cc.business_id = o.id and cc.user_id='"+userid+"' and cc.defkey='"+defkey+"' ";
		String tableType = defkey.split("_")[0];
		if(StrKit.notBlank(searchText)){
			if("ContractApply".equals(tableType)){
				sql = sql + " and (o.applyer_name like '%"+searchText+"%' or o.org_name like '%"+searchText+"%'"
		...
		sql = sql +" order by o.create_time desc";
		return Db.paginate(Integer.valueOf(pageNum), Integer.valueOf(pageSize), "select * ", sql);
	}
	...
	public Record getDocument(String defKey, String id, String pageType) {
		...
		Record newResult = MobileWorkflowService.modifyResult(result, defKey);
		return newResult;
	}
