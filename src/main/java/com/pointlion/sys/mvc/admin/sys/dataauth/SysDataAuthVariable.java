...
public class SysDataAuthVariable {
	...
	public String transRuleVariable(SysDataAuthRuleBean rule){
		...
		if(user!=null){
			//组装value的值---start
			if(value.indexOf(Constants.CurrentUser_Org_Id)>=0){//登陆人机构id
				value = value.replace(Constants.CurrentUser_Org_Id, user.getOrgid());
			}
			if(value.indexOf(Constants.CurrentUser_Org_ParentChildCompanyId)>=0){//登陆人子公司
			...
			if(value.indexOf(Constants.CurrentUser_Id)>=0){//登陆人id
				value = value.replace(Constants.CurrentUser_Id, userid);
			}
			if(value.indexOf(Constants.CurrentUser_Org_AllChildId)>=0){//登陆人递归子公司(这种情况必须是in)
