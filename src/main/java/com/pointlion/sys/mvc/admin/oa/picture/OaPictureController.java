...
public class OaPictureController extends BaseController {
	public static final OaPictureService service = OaPictureService.me;
     ...
     */
    public void save(){
    	OaPicture o = getModel(OaPicture.class);
...
//    		o.setCreateTime(DateUtil.getTime());
    ...
    public void getEditPage(){
    	...
    	if(StrKit.notBlank(id)){//修改
    		OaPicture o = service.getById(id);
    		setAttr("o", o);
    		//是否是查看详情页面
...
//    			}
    	...
    	setAttr("formModelName",StringUtil.toLowerCaseFirstOne(OaPicture.class.getSimpleName()));//模型名称
    	render("edit.html");
    }
    /***
     * 删除
...
//    	o.setProcInsId(insId);
     ...
     * @param defKey
     */
    public void deleteByLinkid(String linkId,String defKey){
    	List<OaPicture> picList = OaPicture.dao.getBylinkidAndDefKey(linkId, defKey);
    	if(picList.size()>0){
    		for (OaPicture pic : picList) {
    			if(StrKit.notNull(pic)){
    	       		 new AliyunOssUtil().delImg(pic.getImgurl());
    	       		 pic.delete();
    			}
			}
