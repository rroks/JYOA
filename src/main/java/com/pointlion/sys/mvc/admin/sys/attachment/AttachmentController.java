...
public class AttachmentController extends BaseController {
	 ...
	 */
	public void attachmentUpload() throws FileNotFoundException{
		String busid = getPara("busid","");
	...
	//	String key = getPara("key","notknowkey");
		...
		String fileType = "";
		fileType = stuf.toLowerCase();
		String newUrl = path+"/"+UuidUtil.getUUID()+"."+stuf;//新文件相对路径
		if(fileType.equals("bmp")||fileType.equals("jpg")||fileType.equals("jpeg")||fileType.equals("png")||fileType.equals("gif")){//图片格式BMP、JPG、JPEG、PNG、GIF
			fileType = "png";
			newUrl = path+"/"+UuidUtil.getUUID()+".png";//新文件相对路径
		}
		String newRealFileUrl = bathPath+newUrl;//文件实际存储路径
		file.getFile().renameTo(new File(newRealFileUrl));//文件重命名
		//信息上传阿里云OSS
		new AliyunOssUtil().uplodaImg("nfwyoa"+newUrl,new FileInputStream(newRealFileUrl));
		if(fileType.equals("png")){
			new File(newRealFileUrl).delete();
		}
		SysAttachment attachment = new SysAttachment();
		attachment.setId(UuidUtil.getUUID());
		attachment.setUrl(newUrl);
		attachment.setRealUrl(new AliyunOssUtil().getPicURL("nfwyoa"+newUrl));
