...
public class SysDct extends BaseSysDct<SysDct> {
	 ...
	 * @return
	 */
	public List<SysDct> getByTypeAndNotGroupid(String type,String tableids){
		String sql = "select * from sys_dct d where d.type='"+type+"'";
		if(StrKit.notBlank(tableids)){
			sql = sql + "and id not in("+ tableids +")";
		}
		sql = sql + "order by d.key asc";
		return dao.find(sql);
	}
}