...
public class SysUser extends BaseSysUser<SysUser> {
	 ...
	 * @return
	 */
	public SysUser getByUserMobile(String mobile){
		return dao.findFirst("SELECT * from sys_user u where u.mobile='"+mobile+"'");
	}
	
	public SysUser getByUserNameOrMobile(String key){
		SysUser user =  dao.findFirst("SELECT * from sys_user u where u.username='"+key+"'");
		if(user != null ){
			return user;
		}else{
			return dao.findFirst("SELECT * from sys_user u where u.mobile='"+key+"'");
		}
	}
	 ...
	 * 递归直到父节点type为"1"
	 */
	public List<SysUser> getUserByOrgidAndRoleKey3(String orgid,String key){
		List<SysUser> list = getUserByOrgidAndRoleKey(orgid,key);
		if(list==null||list.size()==0){
			SysOrg org = SysOrg.dao.getById(orgid);
			if(org!=null&&!"#root".equals(org.getParentId())&&"0".equals(org.getType())){
				list = getUserByOrgidAndRoleKey3(org.getParentId(),key);
			}
		}
		return list;
	}
	 ...
	 * 获取所有用户
	 */
	public List<SysUser> getByNameOrMobile(String sqlTxt){
		return SysUser.dao.find("select * from sys_user u where u.name like concat('%','"+sqlTxt+"','%') or u.mobile like concat('%','"+sqlTxt+"','%')  order by u.name");
	}
	
	public Page<Record> getByNameOrMobilePage(String pageNum, String pageSize, String sqlTxt){
		
//		return Db.paginate(Integer.valueOf(pageNum), Integer.valueOf(pageSize), "select *", " from sys_user u where u.name like concat('%','"+sqlTxt+"','%') or u.mobile like concat('%','"+sqlTxt+"','%')  order by u.name");
		return Db.paginate(Integer.valueOf(pageNum), Integer.valueOf(pageSize), "select u.id, u.email, u.mobile,u.position,u.name,u.username,o.name orgname,o.parent_id,t.name parentname ", " from sys_user u left join sys_org o on o.id = u.orgid left join sys_org t on t.id = o.parent_id  where u.name like concat('%','"+sqlTxt+"','%') or u.mobile like concat('%','"+sqlTxt+"','%')  order by u.name");
	}
	
	public Record getUserAndParentOrg(String userid){
		return Db.findFirst("select u.id, u.name,u.username,o.name orgname,o.parent_id,t.name parentname from sys_user u left join sys_org o on o.id = u.orgid left join sys_org t on t.id = o.parent_id where u.id = '"+userid+"'");
	}
}
